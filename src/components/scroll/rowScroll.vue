<template>
	<div class="rowScroll" :id="box_id">
		<div :id="wrapper_id" class="row-iscroll" :style="{width:list.length*1.55+1+'rem'}">
			<div  v-for="(item,index) in list" :key="index" :style="{color:index==typeIndex?'#B29F74':'#3c3c3c'}" class="light-26" @click="showMe(item,index)">
				{{item.name}}
			</div>
		</div>
	</div>
...
<script>
	...
	export default{
		...
		methods:{
			showMe(item,index){
				if(index == this.typeIndex){return null;}
				var rows = document.getElementById(this.box_id)
				this.$emit('rowChange',item,index)
